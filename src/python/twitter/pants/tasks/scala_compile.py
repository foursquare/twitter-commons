...
class ScalaCompile(NailgunTask):
  ...
  def setup_parser(cls, option_group, args, mkflag):
    ...
    option_group.add_option(mkflag("check_missing_deps"), mkflag("check_missing_deps", negate=True),
                            dest="scala_check_missing_deps",
                            action="callback", callback=mkflag.set_bool,
                            default=False, 
                            help="[%default] Check for undeclared dependencies in scala code")

    option_group.add_option(mkflag("check_all_deps"), mkflag("check_all_deps", negate=True),
                            dest="scala_check_all_deps",
                            action="callback", callback=mkflag.set_bool,
                            default=False, 
                            help="[%default] Check all dependencies in scala code")
  ...
  def execute(self, targets):
    ...
    if scala_targets:
      ...
      if self.check_missing_deps:
        target_deps = self.get_compilation_dependencies(scala_targets)
        for target in target_deps:
          ...
          if len(deps) > 0:
            ...
            # an error.
            print "Error: target %s has undeclared compilation dependencies on %s" % (target, deps)
  ...
  def get_compilation_dependencies(self, targets):
    """
    ...
    for target in targets:
      deps_by_source = {}
      (outdir, depfile, cachedir) = ScalaCompile.create_output_paths(self, [ target ])
      ...
      for cl in target_by_class_for_target:
        ...
        if len(cltargets) > 1:
         ...
         if cl in targets_by_class:
           # error in this case? Probably need to check if the result of this is more than
           # one target?
           targets_by_class[cl] = targets_by_class[cl].union(cltargets)
         else:
           targets_by_class[cl] = set(cltargets)
      ...
      for source in target.sources:
        ...
        # add the source->deps map to a list of target -> (source -> dep) maps
        if target in deps_by_target:
          deps_by_target[target] = deps_by_target[target].union(source_file_deps)
        else:
          deps_by_target[target] = source_file_deps
    ...
    for target in deps_by_target:
      target_dep_classes = deps_by_target[target]
      for cl in target_dep_classes:
        if cl not in targets_by_class:
          ...
          if self.check_all_deps and not reduce(lambda x, y: x or y,
          ...
          continue
        target_dep_targets = targets_by_class[cl]
        if target in computed_deps:
          computed_deps[target] = computed_deps[target].union(target_dep_targets)
        else:
          computed_deps[target] = set(target_dep_targets)
